...
const {gas, gasPrice} = {gas: 4075039, gasPrice: 7000000000};

const FOLDER_NAME = 'spooky_batch_5';
...
async function convertRawToIpfsPayload(data) {
  ...
  // Update IPFS data
  fs.writeFileSync(`./scripts/batch_minter/${FOLDER_NAME}/spooky_ipfs_data.json`, JSON.stringify(rawIpfsData, null, 2));
  ...
  async function loadData() {
    for (let data of rawIpfsData) {
      if (!data['uploaded_ipfs_token_uri']) {
        ...
        fs.writeFileSync(`./scripts/batch_minter/${FOLDER_NAME}/spooky_uploaded_ipfs_data.json`, JSON.stringify(rawIpfsData, null, 2));
      } else {
