...
async function convertRawToIpfsPayload(data) {
  ...
  // Update IPFS data
  fs.writeFileSync('./scripts/batch_minter/spooky_batch_1/spooky_ipfs_data.json', JSON.stringify(rawIpfsData, null, 2));
  ...
  async function loadData() {
    for (let data of rawIpfsData) {
      if (!data['uploaded_ipfs_token_uri']) {
        ...
        fs.writeFileSync('./scripts/batch_minter/spooky_batch_1/spooky_uploaded_ipfs_data.json', JSON.stringify(rawIpfsData, null, 2));
      } else {
